...
def LoadJson(Path: str, DefaultValue):
    if not os.path.exists(Path):
        return DefaultValue
    with open(Path, "r", encoding="utf-8") as f:
        return json.load(f)

def SaveJson(Path: str, Data):
...
def PowRatio(AttackerStat: float, DefenderStat: float) -> float:
    if DefenderStat <= 0:
        return 1.0
    return (AttackerStat / DefenderStat) ** 0.75
...
class BattleEntity:
    ...
    def Stat(self, StatName: str) -> float:
        L = float(self.Level)
        W = float(self.Weights.get(StatName, 0.0))
        return (L * (1.0 + W)) ** 2
    ...
    def HasStatus(self, StatusName: str) -> bool:
        return any(S.Name == StatusName and (S.RemainingTurns > 0 or S.DurationSeconds > 0) for S in self.Statuses)

    def GetDefendMultipliers(self) -> Tuple[float, float, float]:
        if self.HasStatus("Defend"):
            return (DefendDamageTakenMultiplier, DefendRegenMultiplier, DefendDefenseQteWindowMultiplier)
        return (1.0, 1.0, 1.0)

    def ApplyTurnRegen(self):
        if not self.Alive:
            return
        _, RegenMult, _ = self.GetDefendMultipliers()
        HpGain = self.MaxHp() * TurnRegenPercent * RegenMult
        MpGain = self.MaxMp() * TurnRegenPercent * RegenMult
        self.HealHp(HpGain)
        self.RestoreMp(MpGain)

    def BeginLagHp(self, NewValue: float):
        Old = float(self.LagHp)
        Delta = abs(NewValue - Old)
        Den = max(1.0, self.MaxHp())
        Percent = Clamp(Delta / Den, 0.0, 1.0)
        Duration = max(0.05, Percent * FullBarDrainSeconds)
        self.LagHpFrom = Old
        self.LagHpTo = NewValue
        self.LagHpTimer = 0.0
        self.LagHpDuration = Duration

    def BeginLagMp(self, NewValue: float):
    ...
    def TakeDamage(self, Amount: float):
        if not self.Alive:
            return
        DamageTakenMult, _, _ = self.GetDefendMultipliers()
        Amount = max(0.0, Amount) * DamageTakenMult
...
def CreateDefaultData():
    ...
    EntitiesPath = os.path.join(DataFolder, "Entities.json")
    AbilitiesPath = os.path.join(DataFolder, "Abilities.json")
    ItemsPath = os.path.join(DataFolder, "Items.json")
    AreasPath = os.path.join(DataFolder, "Areas.json")
    EncountersPath = os.path.join(DataFolder, "Encounters.json")
    BalancePath = os.path.join(DataFolder, "Balance.json")
    ...
    if not os.path.exists(ItemsPath):
        Items = [
            ...
            {"Name": "Iron Sword", "Value": 120, "Tier": 2, "Description": "A basic blade. (placeholder for equipment later)"},
        ]
        SaveJson(ItemsPath, Items)

    if not os.path.exists(AbilitiesPath):
        Abilities = [
            {"Name": "Attack", "Kind": "Attack", "Targeting": "Enemy Single", "Base Delay": BaselineAbilityDelay,
             "Mult": 1.0, "Base MP Cost": 0, "Description": "Basic attack."},
            {"Name": "Defend", "Kind": "Defend", "Targeting": "Self", "Base Delay": 2.0,
             "Mult": 1.0, "Base MP Cost": 0, "Description": "1-turn buff: -25% damage taken, x2 regen, +25% defense QTE window."},

            {"Name": "Quick Jab", "Kind": "Attack", "Targeting": "Enemy Single", "Base Delay": 1.5,
             "Mult": 0.7, "Base MP Cost": 8, "Description": "Fast, weaker hit."},
            {"Name": "Heavy Strike", "Kind": "Attack", "Targeting": "Enemy Single", "Base Delay": 4.0,
             "Mult": 1.8, "Base MP Cost": 18, "Description": "Slow, strong hit."},
            {"Name": "Rally", "Kind": "Buff", "Targeting": "Ally Single", "Base Delay": 2.8,
             "Mult": 1.0, "Base MP Cost": 14, "Description": "Buff Power for a duration."},
            {"Name": "Focus", "Kind": "Buff", "Targeting": "Self", "Base Delay": 2.6,
             "Mult": 1.0, "Base MP Cost": 12, "Description": "Buff Precision for a duration."},
            {"Name": "Mend", "Kind": "Heal", "Targeting": "Ally Single", "Base Delay": 3.0,
             "Mult": 1.2, "Base MP Cost": 16, "Description": "Heal an ally."},

            {"Name": "Toughness", "Kind": "Passive", "Targeting": "Self", "Base Delay": 0.0,
             "Mult": 1.0, "Base MP Cost": 0, "Description": "Passive. Placeholder passive ability."},
        ]
        SaveJson(AbilitiesPath, Abilities)

    if not os.path.exists(EntitiesPath):
        Entities = [
            {"Name": "Hero", "Level": 6,
             "Weights": {"Vitality": 0.50, "Power": 0.25, "Dexterity": 0.15, "Precision": 0.10},
             "Abilities": ["Attack", "Defend", "Quick Jab", "Heavy Strike", "Mend", "Focus", "Rally", "Toughness"],
             "Learnset": [],
             "Drop Table": []},

            {"Name": "Rogue", "Level": 6,
             "Weights": {"Vitality": 0.42, "Power": 0.23, "Dexterity": 0.25, "Precision": 0.10},
             "Abilities": ["Attack", "Defend", "Quick Jab", "Focus"],
             "Learnset": [],
             "Drop Table": []},

            {"Name": "Green Slime", "Level": 4,
             "Weights": {"Vitality": 0.58, "Power": 0.20, "Dexterity": 0.14, "Precision": 0.08},
            ...
            {"Name": "Goblin", "Level": 5,
             "Weights": {"Vitality": 0.44, "Power": 0.28, "Dexterity": 0.18, "Precision": 0.10},
             "Abilities": ["Attack", "Quick Jab"],
             "Learnset": [],
             "Drop Table": [
    ...
    if not os.path.exists(AreasPath):
        Areas = [
            {"Name": "Starter Field", "Description": "A simple test area.", "Encounters": ["Field Encounter 1", "Field Encounter 2"]}
        ]
        SaveJson(AreasPath, Areas)

    if not os.path.exists(EncountersPath):
        Encounters = [
        ...
        SaveJson(EncountersPath, Encounters)

    if not os.path.exists(BalancePath):
        Balance = {
